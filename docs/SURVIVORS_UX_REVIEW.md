...
### Direct story entry and fixed Mission Control — 23 September
Verified the preceding model audit is complete: 107 tests passed again; its Git commit had been interrupted by an approval-service usage limit. Beyond collapse now offers two direct story entries when the selected threat is absent, rather than four inert cards asking for a starting scenario. Choosing the agent route sets up the agent world automatically only when necessary, announces the change, and retains Undo. Existing qualifying worlds continue directly. Card titles explain the selected defence in Mission Control; answer buttons change the survival ending. Mission Control uses a 65px fixed-height instrument with an overflow-only text ticker, hover/focus pause, full-message tooltip and accessible label. Motion off permits manual horizontal scrolling. Long messages cannot increase the row height.
