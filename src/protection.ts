...
export const settingNames:Partial<Record<keyof Settings,string>>={verification:'Warning check strength',decisionTime:'Time to check a warning',independent:'Separate warning-check channel',reserves:'Hospital backup',repairBackup:'Repair-tool backup',repairSupplies:'Repair materials',supplyDelivery:'Independent deliveries',reach:'Regions sharing the fault',aidStrength:'Help from neighbours',aidBudget:'Neighbour relief stock',independentStop:'Disconnect the agent',screening:'Health-threat screening',trustedChannels:'Trusted emergency messages',repairAssistance:'AI repair advice'};
...
export function protections(s:Settings){return [
 ...
 {id:'refuges',title:'Keep neighbours able to help',patch:{reach:Math.min(s.reach,4),aidStrength:100,aidBudget:168}},
...
export function protectionEffect(before:Result,after:Result){
 ...
 return changes.length?changes.join(' '):after.pathways.harmfulOperation&&after.pathways.controlLost?'Settings changed, but the agent is still undoing repairs. Disconnect it to let the crews make progress.':'Settings changed, but the main care, repair-time and world-outcome results are unchanged in this scenario. Open the changed settings to investigate further.';
}
