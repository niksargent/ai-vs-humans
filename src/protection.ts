...
export const settingNames:Partial<Record<keyof Settings,string>>={checkEffectiveness:'Testing effectiveness',evaluationCapacity:'Update testing capacity',repair:'Repair work per fault',verification:'Warning check strength',decisionTime:'Time to check a warning',independent:'Separate warning-check channel',reserves:'Hospital backup',repairBackup:'Repair-tool backup',repairSupplies:'Repair materials',supplyDelivery:'Independent deliveries',connectedness:'Regional connectedness',aidStrength:'Help from neighbours',aidBudget:'Neighbour relief stock',independentStop:'Disconnect the agent',screening:'Health-threat screening',trustedChannels:'Trusted emergency messages',repairAssistance:'AI repair advice'};
export function settingValue(key:keyof Settings,value:number|boolean){if(key==='researchSpeed'&&typeof value==='number')return `${Math.round(value*6)} updates/month`;if(key==='evaluationCapacity'&&typeof value==='number')return `${value/5} tests/day`;return typeof value==='boolean'?(value?'On':'Off'):`${value}${['reserves','repairBackup'].includes(key)?' hours':key==='decisionTime'?' minutes':['verification','supplyDelivery','aidStrength','trustedChannels'].includes(key)?'%':''}`;}
