...
export interface RecoveryFrame {
  time:number; work:number; rate:number; parts:number; hospitalBackup:number; foodBackup:number;
...
export function* recoveryProcess(s:Settings,incident:boolean,powerAffected:boolean,step=1,stresses?:PathwayFrame[]):Generator<RecoveryFrame,RecoveryResult,number|undefined> {
  ...
  const extended=!!stresses?.some(f=>f.healthcare<1||f.trust<1||f.hostile);
  ...
  while(true){
    ...
    const finished=!incident||work>=s.repair-1e-9;
    ...
    const done=Math.min(rate*dt,s.repair-work);
  ...
  const completed=!incident||work>=s.repair-1e-9;
  ...
  return {completed,restoredAt,horizon:RECOVERY_HORIZON,observedHours:round(time),
    status:!incident?'unneeded':completed?'repaired':frame.rate<1e-9?'stalled':'progressing',progress:!incident?1:Math.min(1,work/s.repair),
