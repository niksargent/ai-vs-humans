...
function enterStage(next:Stage){stage=next;visited.add(next);trail.length=0;selected='';regionContext=null;storyIndex=0;panel=next==='world'?'pathways':next==='chain'?'story':next==='damage'?'outcomes':next==='rescue'?'rescue':'beyond';if(next==='world')focus('origins');if(next==='rescue')focus('recovery');if(next==='damage')focus('health');render();if(next==='chain')focusStory();$('#readout').scrollTop=0;guide(({world:'Follow this world’s story, or load a new spark. Drag a dial up or down to change the ending.',chain:'Follow one link at a time. The circuit travels with you.',damage:'Look at what people lose—and what keeps working. Select a readout to look closer.',rescue:'You have options. Protect a lifeline and watch the chain change.',beyond:'A catastrophe can leave people alive. Explore what would threaten the survivors.'})[next]);}
...
function renderPanel(){
  ...
  if(panel==='outcomes'){
    ...
    el.innerHTML=panelHeader('Human choices')+`<p class="experiment-tag">DIFFERENT HUMAN DECISIONS</p><h2 class="inspector-title">Same warning.<br>Different endings.</h2><p class="inspector-copy">People do not always react the same way to a warning. Keep your controls fixed and replay 256 possible human responses.</p>`+(range?[
...
document.addEventListener('click',e=>{
  ...
  const rescue=target.closest<HTMLElement>('[data-rescue]');if(rescue){const id=rescue.dataset.rescue!;const changes:Record<string,Partial<Settings>>={assistance:{repairAssistance:true},checks:{verification:100,decisionTime:120,independent:true},hospital:{reserves:720},isolation:{independentStop:true},screening:{screening:true},trust:{trustedChannels:100},repair:{repairBackup:720,repairSupplies:168,supplyDelivery:100},refuges:{reach:Math.min(settings.reach,4),aidStrength:100,aidBudget:168}};change({...settings,...changes[id]});return;}
  ...
  const pathwayButton=target.closest<HTMLElement>('[data-pathway]');if(pathwayButton){const id=pathwayButton.dataset.pathway!;mode='case';trail.length=0;change({...DEFAULTS,...pathwayPresets[id]});guide('New experiment loaded. Starting settings replace the previous world. Undo brings your world back.');inspect(({outage:'comms',research:'development',control:'control',health:'bio',information:'information',deliveries:'payments'} as Record<string,string>)[id]);return;}
...
$('#reset-button').addEventListener('click',()=>{clearFeedback();remember();settings={...DEFAULTS};continuation=freshContinuation();monthOptions={...MONTH_DEFAULTS};monthSeed=42;seed=42;result=simulate(settings,seed);selected='';panel='pathways';range=null;requestId++;persist();enterStage('world');$('#change-text').textContent='Opening world restored. Follow its story, then try to keep the hospital working.';});
...
function rescuePanel(){return panelHeader('Your moves')+`<div class="readout-name"><span class="etched-label">THE WORLD IS NOT LOST</span><h2>Break the chain.</h2><p>One well-placed safeguard can change what happens next.</p></div>`+[
 ['checks','Buy time. Check the warning.','Give people an independent check before they act.'],['hospital','Keep the hospital lights on.','Give essential equipment 30 days of backup.'],['repair','Protect the repair crews.','Keep their tools powered and supplies coming.'],['refuges','Keep a lifeline outside the failure.','Keep at least two regions independent so they can send help.'],...(settings.agentEnabled?[['isolation','Take back control.','Disconnect the agent from essential systems.']]:[]),...(settings.healthChallenge?[['screening','Stop the threat before it starts.','Close the screening gate.']]:[]),...(settings.informationCampaign?[['trust','Let trusted voices get through.','Restore reliable emergency instructions.']]:[])
 ].map(([id,title,copy])=>`<button class="rescue-switch" data-rescue="${id}"><span class="switch-light"></span><span><strong>${title}</strong><small>${copy}</small></span><span>↗</span></button>`).join('')+`<button class="small-button" data-rescue="assistance">Let AI help crews find the fault ↗</button><button class="small-button" data-panel="recovery">Look inside the repair effort ↗</button>`;}
