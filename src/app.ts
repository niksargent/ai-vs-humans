...
function description(key:NumericKey):{value:string;unit:string;note:string} {
  switch(key){
    ...
    case 'researchSpeed':return {value:`${(4*settings.researchSpeed/100).toFixed(1)}/day`,unit:settings.researchEnabled?'candidate ideas':'projects paused',note:settings.researchEnabled?`${Math.floor(result.pathways.research.made)} ideas · ${Math.floor(result.pathways.research.checked)} checked in 30 days.`:'Open AI projects to start the research queue.'};
...
function feedback(before:Result){
  ...
  if(key==='researchSpeed'||key==='researchEnabled')guide(settings.researchEnabled?`AI projects running: ${Math.floor(result.pathways.research.made)} ideas, ${Math.floor(result.pathways.research.checked)} checked, ${Math.floor(result.pathways.research.unchecked)} released unchecked. Computers, experiments and checks set the pace.`:'AI projects paused. Other threats and defences are still active.');
...
function collapsePanel(){
  ...
  const title=result.nuclear?'Wider outcome unknown':c.crossedAt!==null?(c.recoveredAt!==null?'Back from the brink.':'The world is coming apart.'):'The world holds together.';
