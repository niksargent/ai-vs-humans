...
function description(key:NumericKey):{value:string;unit:string;note:string} {
  switch(key){
    ...
    case 'researchSpeed':return {value:`${(4*settings.researchSpeed/100).toFixed(1)}/day`,unit:settings.researchEnabled?'updates/day requested':'projects paused',note:settings.researchEnabled?`${(result.pathways.research.made/30).toFixed(1)} updates/day actually produced. Computers and scenarios can limit the pace; ${Math.floor(result.pathways.research.checked+1e-9)} tested in 30 days.`:'Open AI projects to start the research queue.'};
...
function feedback(before:Result){
  ...
  else if(key==='tension')guide('Tension changes how likely people are to escalate. This replay keeps the same human choices. Open Read the damage → Different human decisions to explore other endings.');
...
function renderPanel(){
  ...
  if(panel==='outcomes'){
    ...
    el.innerHTML=panelHeader('World status')+`<div class="readout-name"><span class="etched-label">DAMAGE DURING THIS INCIDENT</span><h2>What happened to this world?</h2></div>`+
      ...
      `<button class="primary-action" data-stage="rescue">Protect the world <span>→</span></button><button class="small-button" data-inspect="collapse">Could this bring civilisation down? ↗</button><button class="small-button" id="open-month">A month of new updates ↗</button><button class="small-button" data-panel="range">Different human decisions ↗</button>`;
    ...
    el.innerHTML=panelHeader('World settings')+`<details class="model-details"><summary>Main dials</summary>${deckInfo.map(d=>rangeField(d.key,d.title,description(d.key).note,d.key==='reserves'?'hours':'')).join('')}</details>`+(monthReturn?'<button class="primary-action" id="open-month">← Return to the month scenario</button>':'')+`<button class="small-button" data-panel="pathways">Choose a scenario ↗</button>`+pathwayControls(rangeField,toggleField)+`<details class="model-details"><summary>Regions, reserves & repairs</summary>`+
      ...
      rangeField('collapseDays','Days required','An scenario definition, not a settled scientific cutoff.','days')+`</details>`+
    ...
    el.innerHTML=panelHeader('Human choices')+`<p class="scenario-tag">DIFFERENT HUMAN DECISIONS</p><h2 class="inspector-title">Same warning.<br>Different endings.</h2><p class="inspector-copy">People do not always react the same way to a warning. Keep your controls fixed and replay 256 possible human responses.</p>`+(range?[
...
document.addEventListener('click',e=>{
  ...
  if(id==='try-wide-failure'){trail.length=0;mode='case';change({...DEFAULTS,reach:6,crews:25,repairBackup:24,foodStores:48,regionDifference:50,responseBackup:72,collapseRegions:4,collapseDays:14});panel='collapse';render();return;}
  if(id==='keep-refuges'){mode='case';change({...settings,reach:Math.min(settings.reach,4),aidStrength:100,aidBudget:168});panel='collapse';render();return;}
