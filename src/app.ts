...
function enterStage(next:Stage){stage=next;visited.add(next);trail.length=0;selected='';regionContext=null;storyIndex=0;panel=next==='world'?'pathways':next==='chain'?'story':next==='damage'?'outcomes':next==='rescue'?'rescue':'beyond';if(next==='world')focus('origins');if(next==='rescue')focus('recovery');if(next==='damage')focus('health');render();if(next==='chain')focusStory();$('#readout').scrollTop=0;guide(({world:'Watch the story, or load a new scenario. Drag a dial up or down to change the ending.',chain:'Follow one link at a time. The circuit travels with you.',damage:'Look at what people lose—and what keeps working. Select a readout to look closer.',rescue:'You have options. Protect a lifeline and watch the chain change.',beyond:'A catastrophe can leave people alive. Explore what would threaten the survivors.'})[next]);}
...
function change(next:Settings,record=true,origin?:Result){protectionReceipt='';cancelAnimationFrame(setupFrame);if(record)remember();const before=origin||result;settings=validateSettings(next);result=simulate(settings,seed);range=null;requestId++;feedback(before);persist();render();}
...
document.addEventListener('click',e=>{
  ...
  if(target.closest('#continuation-start')){$<HTMLDialogElement>('#dialog').close();enterStage('world');setPanel('collapse');guide('Explore widespread service failure, then return to the survivors. The starting-world button replaces your settings; Undo restores them.');return;}
  ...
  const pathwayButton=target.closest<HTMLElement>('[data-pathway]');if(pathwayButton){const id=pathwayButton.dataset.pathway!;if(currentScenario(settings).id===id&&!currentScenario(settings).modified){enterStage('chain');return;}mode='case';trail.length=0;change({...DEFAULTS,...pathwayPresets[id]});guide('New scenario loaded. Starting settings replace the previous world. Undo brings your world back.');inspect(({outage:'comms',research:'development',control:'control',health:'bio',information:'information',deliveries:'payments'} as Record<string,string>)[id]);return;}
  ...
  if(id==='new-case'){protectionReceipt='';clearFeedback();remember();seed=(seed+1)>>>0;range=null;requestId++;result=simulate(settings,seed);panel='outcomes';mode='case';persist();render();$('#change-text').textContent=`A different set of regional links and human decisions. Your settings stay the same.`;}
...
document.addEventListener('keydown',e=>{const el=e.target as Element;if(el.matches('[data-node]')&&['Enter',' '].includes(e.key)){e.preventDefault();inspect((el as SVGElement).dataset.node!);}if(e.key==='Escape'&&!document.querySelector('#quick-tour')&&!$<HTMLDialogElement>('#dialog').open){back();}});
$('#readout').addEventListener('input',e=>{const input=e.target as HTMLInputElement;if(!input.dataset.setting)return;protectionReceipt='';const key=input.dataset.setting as keyof Settings;const before=result;remember();settings={...settings,[key]:input.type==='checkbox'?input.checked:Number(input.value)};result=simulate(settings,seed);range=null;requestId++;persist();if(input.type==='range')input.previousElementSibling!.querySelector('output')!.textContent=`${input.value} ${input.dataset.unit}`;feedback(before);render(true);});
...
$('#undo-button').addEventListener('click',()=>{const old=history.pop();if(!old)return;protectionReceipt='';clearFeedback();settings=old.settings;seed=old.seed;continuation=old.continuation;monthOptions=old.monthOptions;monthSeed=old.monthSeed;result=simulate(settings,seed);range=null;requestId++;persist();render();$('#change-text').textContent="Your last change is undone. You are back where you were.";});
$('#reset-button').addEventListener('click',()=>{protectionReceipt='';clearFeedback();remember();settings={...DEFAULTS};continuation=freshContinuation();monthOptions={...MONTH_DEFAULTS};monthSeed=42;seed=42;result=simulate(settings,seed);selected='';panel='pathways';range=null;requestId++;persist();enterStage('world');$('#change-text').textContent='Opening scenario restored. Watch the story, then try to keep the hospital working.';animateSetup();});
...
const tourSteps=[
 ...
 {target:'#world-settings-button',title:'All the settings live here',copy:'Some protections change settings beyond the seven dials. Open World settings to find them. A setting in a protection receipt takes you straight to that control.'}
];
