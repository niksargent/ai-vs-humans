...
export interface Result {
  ...
  verificationMinutes:number; events:{id:string; time:number; parents:string[]; description:string}[];
}
...
export function simulate(input:Settings,seed=42):Result {
  ...
  const regions=civilisation.regions.map(r=>({name:r.name,comms:r.exposed,
    ...
    food:r.recovery.foodGap>0,emergency:r.recovery.emergencyGap>0}));
  ...
  const timely=verificationMinutes<=s.decisionTime;
  ...
  set('checks',verified?'safe':warning?'exposed':'quiet',verified?'Warning challenged':warning?(timely?'Check missed the error':'Check arrives too late'):'No warning to check',
    warning?`Verification needs ${verificationMinutes} minutes; the decision window is ${s.decisionTime} minutes. ${verified?'The check succeeds in this case.':timely?'The check is timely but does not catch this case.':'A check cannot protect this decision after its deadline.'}`:'The AI-mediated warning route was not reached.', ['comms'],`Verification time = 90 − 0.65 × check strength, plus up to 60 minutes if checks depend on failed communications. A timely check succeeds when its fixed event draw is below check strength / 100. All are illustrative assumptions.`);
  ...
  set('crews',!incident?'quiet':lowestCrew>=.99?'safe':lowestCrew===0?'harm':'exposed',`Crew low point · ${Math.round(lowestCrew*100)}%`,
    ...
    'Crew capacity = staffing × (0.6 + 0.4 × hospital support) × (0.5 + 0.5 × food support) × (0.8 + 0.2 × emergency support). Coefficients are authored assumptions, not worker mortality or measured disaster estimates.');
  ...
  set('development',!s.researchEnabled?'quiet':pathways.research.unchecked>=1?'exposed':'safe',!s.researchEnabled?'Research loop off':`${Math.floor(pathways.research.unchecked)} unchecked releases`,
    `In the 30 days before this crisis, AI proposes ${pathways.research.made.toFixed(0)} updates. People check ${pathways.research.checked.toFixed(0)}; ${pathways.research.backlog.toFixed(0)} wait. Checks catch the introduced faulty update in this experiment. Faster research alone does not give AI more access.`,[],
  ...
  set('bio',pathways.healthIntroduced?(Math.min(...pathways.frames.map(f=>f.healthcare))<.5?'harm':pathways.healthGapHours?'exposed':'safe'):s.healthChallenge?'safe':'quiet',pathways.healthIntroduced?`${pathways.healthGapHours}h demand above capacity`:s.healthChallenge?'Health threat blocked':'Health route off',
    pathways.healthIntroduced?`More people need care at the same time. Peak demand reaches ${pathways.peakDemand.toFixed(1)} times normal. Hospitals also need power; illness reduces the available workforce. No deaths are calculated.`:'This route needs an introduced threat, AI scientific assistance, a malicious actor, physical access and failed screening. Remove any gate and this introduced threat is blocked.',[],
    'An abstract extra-demand pulse grows over 168 hours until the response starts, then falls to zero over at most 168 hours. Care support = min(1, (1 + surge capacity) / demand), further limited by hospital power. Workforce falls by up to 40%. This is not a pathogen or epidemic forecast.');
  for(const [id,title] of [['payments','Payments'],['transport','Transport']] as const){
    ...
    set(id,low<.5?'harm':low<1?'exposed':'safe',`${Math.round(low*100)}% at lowest`,
  ...
  set('emergency',emergencyGap?(s.fallback>0?'exposed':'harm'):'safe',emergencyGap?`Help delayed · ${emergencyGap}h`:'Help can get through',
  ...
  return {pathways,civilisation,recoveryModel,nodes,seed,settings:s,incident,powerOutage,hospitalGap,restoreHours,warning,verified,escalation,nuclear,verificationMinutes,events,foodGap,emergencyGap,affectedRegions,regions};
}
export function summariseChange(before:Result,after:Result):string {
  ...
  if(before.foodGap!==after.foodGap)return after.foodGap?`Food warehouses run out of backup ${after.foodGap} hours before power returns.`:'Food warehouses now have enough backup to keep their fridges running.';
