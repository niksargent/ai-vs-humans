...
export const controls = {
  ...
  researchSpeed:{min:0,max:100,step:1/6},computeCapacity:{min:0,max:100,step:5},experimentCapacity:{min:0,max:100,step:5},evaluationCapacity:{min:0,max:100,step:1},
...
export function simulate(input:Settings,seed=42):Result {
  ...
  const commsHours=recoveryModel.frames.filter(f=>f.time<720&&f.communications<1).length;
  let commsRun=0,commsLongest=0;
  for(const f of recoveryModel.frames){if(f.time>=720)break;commsRun=f.communications<.5?commsRun+1:0;commsLongest=Math.max(commsLongest,commsRun);}
  const commsStatus:Status=!commsHours?'safe':commsLongest>=24?'harm':'exposed';
  set('comms',commsStatus,!commsHours?'Calls keep working':commsStatus==='harm'?`Calls fail · ${commsHours}h total`:`Calls strained · ${commsHours}h total`,
    pathways.harmfulOperation&&incident?'The agent disrupts the network and keeps undoing repairs while it remains active. Calls and messages fail in the exposed regions.':incident?`${updates.releases.filter(r=>r.fault).length} faulty updates reach the network this month. ${commsHours?`Communications are disrupted for ${commsHours} hours in Region 1; ${affectedRegions} regions receive the faults.`:`Independent communications keep calls working despite the network faults.`} ${recoveryModel.completed?`Crews finish repairs after ${restoreHours} hours.`:'It is still down after 30 days.'}`:'No faulty update reaches the network this month. Calls and messages keep working.', ['access','development','control'],'Lamp: green means communications stay available; amber means some disruption, but no full day continuously below half capacity; red means at least 24 continuous hours below half capacity in Region 1. This is a display severity threshold, not a probability. Incident = faulty proposal AND network capability AND execution permission. Repair work accumulates each hour, limited by crews, communications, power and consumable supplies. Service failures affect later work. See recovery details.');
